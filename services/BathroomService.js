...
let bathroomLevel = 0;
const _hungerChannelId = "1198326193984913470";
const _hasMessageBeenSent = false;

async function instantiate(_client, _openai) {
...
const BathroomService = {
  ...
  increaseBathroomLevel(_amount = 1) {
    let currentBathroomLevel = BathroomService.getBathroomLevel();
    currentBathroomLevel =
      currentBathroomLevel < 100 ? currentBathroomLevel + 1 : 100;
    BathroomService.setBathroomLevel(currentBathroomLevel);
  ...
  decreaseBathroomLevel() {
    let currentBathroomLevel = BathroomService.getBathroomLevel();
    currentBathroomLevel =
      currentBathroomLevel > 0 ? currentBathroomLevel - 1 : 0;
    BathroomService.setBathroomLevel(currentBathroomLevel);
    console.log(`Bathroom level decreased to: ${currentBathroomLevel}`);
    return currentBathroomLevel;
  },
  async drink(message) {
    const currentBathroomLevel = BathroomService.decreaseBathroomLevel();
    const content = `
        ...
        ${message.content}
        `;
    const response = await DiscordService.generateTextFromSystemUserMessages(
      content,
      message,
    );
