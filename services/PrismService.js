...
export default class PrismService {
   ...
   * @param {object} payload
   * @param {string} payload.id - Unique conversation identifier
   * @param {string} payload.title - Conversation title
   * @param {Array}  payload.messages - Array of { role, name?, content } messages
   * @param {string} [payload.systemPrompt=""]
  ...
  static async saveConversation({
    ...
    return PrismService._request("/conversations", {
      body: { id, title, messages, systemPrompt, settings },
      username,
    });
  }
   ...
   * @returns {Promise<object>}
   */
  static async finalizeConversation({
    id,
    title,
    systemPrompt = "",
    settings = {},
