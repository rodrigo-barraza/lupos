...
class UserRecordingStream {
  ...
  async processAudioChunk(chunk) {
    ...
    const hasAudio =
      audioStats.rms > this.VOICE_THRESHOLD || audioStats.maxAmplitude > 1500;
    const isSilent = audioStats.rms < this.MAX_SILENCE_RMS;

    // Debug logging every 50 frames (~1 second)
    if (this.frameCount % 50 === 0) {
      console.log(
        `[${this.username}] RMS: ${audioStats.rms.toFixed(0)}, ` +
    ...
    if (hasAudio && !this.isSpeaking) {
      ...
      console.log(
        `[${this.username}] Speech started at ${new Date().toISOString()}`,
      );
    } else if (this.isSpeaking) {
      // Currently speaking
      this.currentSegmentBuffer = Buffer.concat([
      ...
      if (isSilent) {
        ...
        if (this.silenceFrames >= this.SILENCE_THRESHOLD) {
          ...
          console.log(
            `[${this.username}] Speech ended. Duration: ${duration}ms, ` +
  ...
  async transcribeSegment(audioBuffer, speechStartTime, segmentNumber) {
    ...
    try {
      ...
      if (this.processedSegments.has(segmentId)) {
        console.log(
          `[${this.username}] Skipping duplicate segment ${segmentId}`,
        );
        return;
      }
      ...
      console.log(
        `[${this.username}] Saving segment ${segmentNumber}, ` +
      ...
      if (mp3Stats.size < 1000) {
        ...
        console.log(
          `[${this.username}] Skipping small file (${mp3Stats.size} bytes)`,
        );
      ...
      if (transcription && transcription.trim().length > 0) {
        ...
        const isDuplicate = transcriptions.some(
          (t) =>
            ...
            (t.userId === this.userId && t.segmentNumber === segmentNumber), // Same user and segment number
        );

        if (!isDuplicate) {
          transcriptions.push(transcriptionEntry);
          console.log(
            `[${this.username}] Transcribed segment ${segmentNumber}: "${transcription.trim()}"`,
          );
        } else {
          console.log(
            `[${this.username}] Skipping duplicate transcription for segment ${segmentNumber}`,
          );
        }
      }
      ...
      console.error(
        `[${this.username}] Error transcribing segment ${segmentNumber}:`,
        error,
      );
    } finally {
      this.processingSegment = false;
  ...
  async finalize() {
    ...
    console.log(
      `[${this.username}] Finalizing. Speaking: ${this.isSpeaking}, ` +
        `Buffer size: ${this.currentSegmentBuffer.length}`,
    );
...
function createRecordingStream(receiver, userId, recordingsDir, guild) {
  ...
  try {
    ...
    const userRecordingStream = new UserRecordingStream(
      userId,
      username,
      recordingsDir,
    );
    ...
    opusStream.pipe(transcriptionDecoder).on("data", (chunk) => {
  ...
  async recordVoiceInVoiceChannel2(interaction, seconds) {
    ...
    if (isRecording && recordingStreams.size > 0) {
      return interaction.reply(
        "🔴 Already recording! Please wait for the current recording to finish.",
      );
    }
    ...
    const replyMessage = await interaction.reply(
      `🔴 Recording started for the next **${seconds} seconds**! Transcribing speech...`,
    );
        ...
        if (audioMixer) {
          ...
          if (stats.size > 0) {
            try {
              ...
              const userTags = voiceChannel.members
                .filter(
                  (member) => member.user.id !== interaction.client.user.id,
                )
              ...
              const embed = new EmbedBuilder()
                ...
                .setDescription(
                  `Requested by <@${interaction.user.id}> in <#${voiceChannel.id}>`,
                )
                .addFields(
                  {
                    name: "Users Recorded",
                  ...
                  },
                  {
                    name: "Transcription",
                    value:
                      formatTranscriptions().substring(0, 1024) ||
    ...
    this.mixInterval = setInterval(() => {
      ...
      if (!this.isStopping || hasBufferedData) {
        if (hasActiveAudio) {
          ...
          const activeCount = Array.from(this.sources.values()).filter(
            (s) => s.isActive,
          ).length;
          if (activeCount > 1) {
            for (let i = 0; i < this.bufferSize; i += 2) {
              ...
              mixed.writeInt16LE(
                Math.floor(sample / Math.sqrt(activeCount)),
                i,
              );
            }
          }
