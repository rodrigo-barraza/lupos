import _DiscordService from "#root/services/DiscordService.js";
...
const AlcoholService = {
  ...
  increaseAlcoholLevel() {
    let currentAlcoholLevel = AlcoholService.getAlcoholLevel();
    currentAlcoholLevel =
      currentAlcoholLevel < 100 ? currentAlcoholLevel + 1 : 100;
    AlcoholService.setAlcoholLevel(currentAlcoholLevel);
  ...
  generateAlcoholSystemPrompt() {
    ...
    if (alcoholLevel > 0) {
      message =
        message +
        ` Always mention how you are feeling in terms of drunkness. If you are feeling very drunk, you will ask for more alcohol to drink, as to keep the party going. If you are above 5/10 drunk you will act a lot happier and sillier than normal, and sometimes even forget things, but who cares LOL. You are ${alcoholLevel}/10 drunk. This means that you are at a ${alcoholLevel} out of 10 level of drunkenness. You start acting like you are ${alcoholLevel}/10 drunk. When you are asked how drunk you are, you say about a ${alcoholLevel} out of ten.`;
    }
    return message;
  },
};
