...
const MoodService = {
  instantiate() {
    ...
    const _currentMood = moods.find((mood) => mood.level === moodLevel);
  ...
  onMoodLevelChange() {
    ...
    client.user.setActivity(
      `Mood: ${currentMood.emoji} ${currentMood.name} (${moodLevel})`,
      { type: ActivityType.Custom },
    );
  },
  increaseMoodLevel(multiplier = 1) {
    let currentMoodLevel = MoodService.getMoodLevel();
    const amountToIncrease = 1 * multiplier;
    currentMoodLevel =
      currentMoodLevel + amountToIncrease < 10
        ? currentMoodLevel + amountToIncrease
        : 10;
    MoodService.setMoodLevel(currentMoodLevel);
  ...
  decreaseMoodLevel(multiplier = 1) {
    let currentMoodLevel = MoodService.getMoodLevel();
    const amountToDecrease = 1 * multiplier;
    currentMoodLevel =
      currentMoodLevel - amountToDecrease > -10
        ? currentMoodLevel - amountToDecrease
        : -10;
    MoodService.setMoodLevel(currentMoodLevel);
  ...
  async generateMoodMessage(message) {
    ...
    if (moodTemperature <= -10) {
      MoodService.decreaseMoodLevel(6);
    } else if (moodTemperature >= -9 && moodTemperature <= -8) {
      MoodService.decreaseMoodLevel(5);
    } else if (moodTemperature >= -7 && moodTemperature <= -6) {
      MoodService.decreaseMoodLevel(4);
    } else if (moodTemperature >= -5 && moodTemperature <= -4) {
      MoodService.decreaseMoodLevel(3);
    } else if (moodTemperature >= -3 && moodTemperature <= -2) {
      MoodService.decreaseMoodLevel(2);
    } else if (moodTemperature == -1) {
      MoodService.decreaseMoodLevel(1);
    } else if (moodTemperature == 0) {
      // No mood change for neutral temperature
    } else if (moodTemperature == 1) {
      MoodService.increaseMoodLevel(1);
    } else if (moodTemperature >= 2 && moodTemperature <= 3) {
      MoodService.increaseMoodLevel(2);
    } else if (moodTemperature >= 4 && moodTemperature <= 5) {
      MoodService.increaseMoodLevel(3);
    } else if (moodTemperature >= 6 && moodTemperature <= 7) {
      MoodService.increaseMoodLevel(4);
    } else if (moodTemperature >= 8 && moodTemperature <= 9) {
      MoodService.increaseMoodLevel(5);
    } else if (moodTemperature >= 10) {
      MoodService.increaseMoodLevel(6);
    }
