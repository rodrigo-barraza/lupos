...
let thirstLevel = 0;
const hungerChannelId = "1198326193984913470";
let hasMessageBeenSent = false;

async function instantiate(client, _openai) {
  const _channel = client.channels.cache.get(hungerChannelId);
...
const ThirstService = {
  ...
  increaseThirstLevel() {
    let currentThirstLevel = ThirstService.getThirstLevel();
    currentThirstLevel =
      currentThirstLevel < 100 ? currentThirstLevel + 1 : 100;
    ThirstService.setThirstLevel(currentThirstLevel);
