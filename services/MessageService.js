...
const MessageService = {
    ...
    generateCurrentConversationUser(message) {
        const username = message?.author?.displayName || message?.author?.username || message?.user?.username;
        const userId = message.author?.id || message.user?.id;
        if (username && userId) {
            const generatedMessage = `You are in a conversation with, and replying directly to ${username}, but there are other people in the chat. You end your response by mentioning ${UtilityLibrary.capitalize(username)}'s name or tagging them on discord as: <@${userId}>.\n`;
