...
const ActionService = {
    async eat(interaction) {
        ...
        if (toxicFoods.includes(getRandomFood)) {
            ...
            systemContent = `
                ...
                Always include this food emoji "${getRandomFood}" in your reply, and no other emoji.
    ...
    async drink(interaction) {
        ...
        `;
        const userContent = `Here, have some ${getRandomFood}.`
        const response = await AIWrapper.generateInCharacterResponse(systemContent, userContent, interaction);
