...
const LocalAIWrapper = {
  async generateLocalAITextResponse(
    ...
    const mergedData = conversation.reduce(
      (accumulator, value, index, array) => {
        if (value.role === "system") {
          ...
          if (
            accumulator.length &&
            accumulator[accumulator.length - 1].role === value.role
          ) {
            if (value.role === "user" && index === array.length - 1) {
              ...
              if (
                ...
                accumulator[accumulator.length - 1].role !==
                  array[index + 1].role
              ) {
    ...
    const response = await fetch(
      `${LOCAL_LANGUAGE_MODEL_API_URL}/v1/chat/completions`,
      {
        method: "POST",
