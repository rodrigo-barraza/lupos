...
async function processReminders(client, mongo) {
  ...
  try {
    ...
    while (true) {
      // Loop to process all due reminders one by one
      ...
      const channel = DiscordUtilityService.getChannelById(
        client,
        userReminder.channelId,
      );

      try {
        if (!channel) {
          console.log(
            `Channel ${userReminder.channelId} not found for reminder ${userReminder._id}`,
          );
        ...
        if (member) {
          ...
          const response = await AIService.generateReminderTextReminderResponse(
