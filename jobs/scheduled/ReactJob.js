...
async function generateReactors(client, _mongo) {
  const channel1 = DiscordUtilityService.getChannelById(
    client,
  ...
  try {
    ...
    mostCommonOverReactor = userIds.reduce(
      (acc, user) => (user.count > acc.count ? user : acc),
      userIds[0],
    );

    if (mostCommonOverReactor?.count > 4) {
      const overReactorRole = guild.roles.cache.find(
        (role) => role.id === overReactorRoleId,
      );
      const currentOverReactor = await guild.members.fetch(
        mostCommonOverReactor.id,
      );

      const membersWithRole = guild.members.cache.filter((member) =>
        member.roles.cache.some((role) => role.id === overReactorRoleId),
      );
      if (previousOverReactorId !== currentOverReactor.id) {
        ...
        await Promise.all(
          membersWithRole.map(async (member) => {
            consoleLog(
              "=",
              `Removing ${overReactorRole.name} role from member: ${member.user.tag} (ID: ${member.id})`,
            );
      ...
      await membersWithRole.forEach((member) =>
        member.roles.remove(overreactorRole),
      );
    }
    consoleLog(">");
  } catch (err) {
