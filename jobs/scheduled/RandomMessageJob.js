...
async function sendRandomMessageTimeout(client) {
    ...
    try {
      if (!channel) {
        ...
        const fetchRecentMessages = (
          await DiscordUtilityService.fetchMessages(client, channelId, {
            limit: 100,
          })
        ).reverse();
        const recentMessages = fetchRecentMessages.map((msg) => msg);
        const lastMessage = recentMessages[recentMessages.length - 1];
        const imageToGenerate = lastMessage.content;
        if (lastMessage.author.id !== client.user.id) {
          consoleLog(
            "=",
            `Sending reminder message to channel: ${channel.id}...`,
          );
          sendTypingInterval =
      ...
      consoleLog(
        "=",
        `Next reminder scheduled in ${(nextDelay / (60 * 60 * 1000)).toFixed(2)} hours (at ${nextRunDate.toLocaleString()})`,
      );
  ...
  consoleLog(
    "=",
    `First reminder scheduled in ${(firstDelay / (60 * 1000)).toFixed(2)} minutes`,
  );
  consoleLog(">");
