...
const { _testHelpers: h } = await import('../../commands/utility/deathrollUtils.js');
...
describe('computePlayerProfile', () => {
    ...
    test('computes winRate correctly', () => {
        const profile = h.computePlayerProfile({ wins: 7, losses: 3, totalGames: 10, mmr: 1100, rd: 100 });
        expect(profile.winRate).toBe(70);
    });

    test('uses passed-in MMR and RD (stored state, not derived)', () => {
        const profile = h.computePlayerProfile({ wins: 5, losses: 5, mmr: 1234, rd: 80 });
    ...
    test('includes streak data', () => {
        const profile = h.computePlayerProfile({ currentStreak: 5, bestStreak: 8, mmr: 1000, rd: 100 });
...
describe('getMultiplierName', () => {
    test('returns named multiplier for known values', () => {
        ...
        expect(h.getMultiplierName(1024)).toBe('Milliduoquattuorsexagesimal (1024x)');
    });
